...
const AddWeddingTheme = () => {
    ...
    const handleSubmit = async (ev) => {
        ...
        const invalidFields = emptyThemeFields(themeName, description, file);
        setInvalidFieldsValue({ ...invalidFields });

        if(Object.values(invalidFields).length === 0) {
            try {
                const form = new FormData(ev.target);
                const createThemeResponse = await sendForm('/api/themes/wedding', form);
    ...
    return (
        <section className="flex flex-col gap-2 p-4">
            ...
            <div className="flex justify-between items-center p-1 rounded-lg">
                <h2 className="font-headings font-semibold">Add New Wedding Theme</h2>
            </div>
            <form onSubmit={ handleSubmit } className="flex gap-4 font-paragraphs">
                ...
                <div className="grow flex flex-col gap-4 py-6">
                    <div className="w-full">
                        ...
                        <input name="themename" onChange={(e) => setThemeName(e.target.value)} className="input w-full border border-neutral-500/40" placeholder="Add Wedding Theme Name" />
...
export default AddWeddingTheme;
