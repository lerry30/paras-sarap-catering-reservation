...
const ViewDebutThemes = () => {
    ...
    const onDeleteDebutTheme = async () => {
        const _k = zDebutTheme.getState().id;
        if(!_k) return;
        setDeletionPrompt(false);
        setLoading(true);

        const response = await deleteWithJSON('/api/themes/debut', { _k: _k });
        if(response?.success) {
            setActionSuccessMessage('Debut theme removed successfully.');
            setTimeout(() => setActionSuccessMessage(''), 2000); // to hide modal
            router.push('/admin?display=debutthemes');
        }
    }

    const onUpdateDebutTheme = () => {
        // I'm not saving any data into store since it is already exist
        router.push('/admin?display=updatedebuttheme');
    }

    useEffect(() => {
        setThemeName(zDebutTheme.getState().name);
        setDescription(zDebutTheme.getState().description);
    }, []);
    ...
    return (
        <>
                ...
                <div className="flex flex-col gap-2 pr-[var(--page-x-padding)]">
                    ...
                    <div className="flex gap-4">
                        <div className="size-96 min-w-96 flex justify-center items-center rounded-md shadow-lg cursor-pointer border border-neutral-500/40 relative">
                            <Image 
                                src={ zDebutTheme.getState().filename }
                                alt=''
                                width={ 400 }
                                height={ 400 }
                                sizes="100%"
                        ...
                        <div className="grow flex flex-col gap-4">
                            ...
                            <div className="w-full flex gap-4">
                                <button onClick={ onUpdateDebutTheme } className="w-full button shadow-md border border-neutral-500/40 bg-emerald-500/40">Update</button>
            ...
            {
                deletionPrompt && <Prompt callback={ onDeleteDebutTheme } onClose={ () => setDeletionPrompt(false) } header="Confirm Drink Removal" message="Are you sure you want to remove this theme? Removing it will completely erase all data associated with it and cannot be undone."/>
            }
...
export default ViewDebutThemes;
