    ...
    const handleSubmit = async (ev) => {
        ...
        if(Object.values(invalidFields).length === 0) {
            try {
                ...
                form.append('id', zDebutTheme.getState().id);
                ...
                const createThemeResponse = await sendFormUpdate('/api/themes/debut', form);
    ...
    return (
        <section className="flex flex-col gap-2 p-4">
            ...
            <div className="flex justify-between items-center p-1 rounded-lg">
                <h2 className="font-headings font-semibold">Update Debut Theme</h2>
            </div>
            <form onSubmit={ handleSubmit } className="flex gap-4 font-paragraphs">
                <div className="flex flex-col gap-4">
                    <UploadButton fileData={ [ file, setFile ]} className="size-95" initialImageSrc={ zDebutTheme.getState().filename } />
                ...
                <div className="grow flex flex-col gap-4 py-6">
                    <div className="w-full">
                        <label className="font-paragraph text-sm font-semibold">Debut Theme Name</label>
                        <input name="themename" value={themeName} onChange={(e) => setThemeName(e.target.value)} className="input w-full border border-neutral-500/40" placeholder="Update Debut Theme Name" />
...
export default UpdateDebutTheme;
