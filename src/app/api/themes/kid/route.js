...
import KidTheme from '@/models/KidThemes';
...
export const GET = async () => {
    try {
        const kidThemes = (await KidTheme.find({}).sort({ createdAt: -1 })) || [];
        const data = [];
        for(const theme of kidThemes) {
...
export const POST = async (request) => {
    try {
        ...
        const kidThemeFromDb = await KidTheme.findOne({ name: fThemeName });
        if(kidThemeFromDb)
            return Response.json({ message: 'Kid Theme already exist', errorData: 'themename' }, { status: 400 }); // I turn 204 to 400 since nextjs have problem with responding with status code of 204 right now
    
        const sharpImageData = await sharp(buffer).jpeg({ quality: imageQuality }).toBuffer(); // reduce file size
        const { uploaded, filename } = await uploadFileToS3(sharpImageData, 'kidthemes'); // upload into aws

        if(!uploaded) 
            return NextResponse.json({ message: 'There\'s something wrong, uploading file to AWS!', errorData: 'unauth' }, { status: 400 });

        const kidtheme = await KidTheme.create({ name: fThemeName, description: fDescription, filename });
...
export const DELETE = async (request) => {
    try {
        ...
        const deletionResponse = await KidTheme.findByIdAndDelete(_id); // return deleted item
...
export const PUT = async (request) => {
    try {
        ...
        if(isImageChange) {
            ...
            if(!uploaded) 
                return NextResponse.json({ message: 'There\'s something wrong, uploading file to AWS!', errorData: 'unauth' }, { status: 400 });
            const prevTheme = await KidTheme.findByIdAndUpdate(themeIdDecoded, { filename }, { option: true });
        ...
        const theme = await KidTheme.findByIdAndUpdate(themeIdDecoded, { name: fThemeName, description: fDescription });
        return NextResponse.json({ message: '', success: true }, { status: 200 });
    } catch(error) {
        console.log(error);
        return NextResponse.json({ message: 'There\'s something wrong!', errorData: 'unauth' }, { status: 400 });
