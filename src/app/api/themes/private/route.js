...
import PrivateTheme from '@/models/PrivateThemes';
...
export const GET = async () => {
    try {
        const privateThemes = (await PrivateTheme.find({}).sort({ createdAt: -1 })) || [];
        const data = [];
        for(const theme of privateThemes) {
...
export const POST = async (request) => {
    try {
        ...
        const privateThemeFromDb = await PrivateTheme.findOne({ name: fThemeName });
        if(privateThemeFromDb)
            return Response.json({ message: 'Private Theme already exist', errorData: 'themename' }, { status: 400 }); // I turn 204 to 400 since nextjs have problem with responding with status code of 204 right now
    
        const sharpImageData = await sharp(buffer).jpeg({ quality: imageQuality }).toBuffer(); // reduce file size
        const { uploaded, filename } = await uploadFileToS3(sharpImageData, 'privatethemes'); // upload into aws

        if(!uploaded) 
            return NextResponse.json({ message: 'There\'s something wrong, uploading file to AWS!', errorData: 'unauth' }, { status: 400 });

        const privatetheme = await PrivateTheme.create({ name: fThemeName, description: fDescription, filename });
...
export const DELETE = async (request) => {
    try {
        ...
        const deletionResponse = await PrivateTheme.findByIdAndDelete(_id); // return deleted item
...
export const PUT = async (request) => {
    try {
        ...
        if(isImageChange) {
            ...
            if(!uploaded) 
                return NextResponse.json({ message: 'There\'s something wrong, uploading file to AWS!', errorData: 'unauth' }, { status: 400 });
            const prevTheme = await PrivateTheme.findByIdAndUpdate(themeIdDecoded, { filename }, { option: true });
        ...
        const theme = await PrivateTheme.findByIdAndUpdate(themeIdDecoded, { name: fThemeName, description: fDescription });
        return NextResponse.json({ message: '', success: true }, { status: 200 });
    } catch(error) {
        console.log(error);
        return NextResponse.json({ message: 'There\'s something wrong!', errorData: 'unauth' }, { status: 400 });
