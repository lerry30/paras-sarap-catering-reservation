...
const ReviewBudget = () => {
    ...
    const [ totalPlayment, setTotalPayment ] = useState(0);
    ...
    const checkNoOfGuest = () => {
        if(toNumber(noOfGuest) === 0) {
            setInvalidFieldsValue({ noofguest: 'Enter a numerical value greater than zero for the number of guest' });
            return;
        }
    ...
    const guestNoInput = (ev) => {
        ...
        if(isNaN(Number(value))) {
            setInvalidFieldsValue(prev => ({ ...prev, noofguest: 'Please enter a numerical value for the number of guest.' }));
            return;
        }
    ...
    return <section className="">
        ...
        <main className="flex flex-col gap-10 divide-y-[1px] px-page-x py-4">
            <section className="flex flex-col gap-4 py-2">
                ...
                <div className="flex gap-6">
                    ...
                    <div>
                        ...
                        {
                            venue?.maximumSeatingCapacity && <article>
            ...
            <div className="w-full flex flex-col md:flex-row gap-4 justify-between py-32">
                <div className="flex flex-col gap-4">
                    <div className="flex gap-4 text-xl">
                        <h3 className="font-headings">Total Payment: </h3>
                        <span className="font-paragraphs font-semibold">{ pesoFormatter.format(totalPlayment) }</span>
                    </div>
                    <p className="font-paragraphs text-sm bg-blue-500/40 text-blue-900 p-4 rounded-lg">The cost is 30 pesos per guest for table and chair rental service.</p>
                </div>
                <div className="w-full sm:w-1/2 flex flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                        <button onClick={ checkNoOfGuest } className="w-full button shadow-md border border-neutral-500/40">Confirm</button>
                        <button onClick={ (ev) => router.push(`/`) } className="w-full button shadow-md border border-neutral-500/40">
                            Cancel
                        </button>
                    </div>
                </div>
