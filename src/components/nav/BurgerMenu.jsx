...
const BurgerMenu = ({ size='w-[30px]', className='', children }) => {
    ...
    return (
        ...
        <div ref={ sidebar } className={`relative ${ className }`}>
            ...
            <div className={ `fixed left-0 w-[230px] h-screen flex flex-col gap-2 mt-1 p-2 pt-6 sm:px-page-x sm:w-[300px] border-r border-neutral-300 shadow-sm bg-white overflow-y-auto transition-transform ${ className } ${ open ? 'translate-x-0' : '-translate-x-full' }` }>
                { children }
            </div>
        </div>
    );
